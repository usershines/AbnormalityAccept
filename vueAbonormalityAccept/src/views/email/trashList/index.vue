<template>
  <el-card style="margin: 10px; border: none; box-shadow: none;">
    ...
    <el-divider content-position="left">垃圾箱 ({{ emails.length }})</el-divider>
    ...
    >
      ...
      <el-table-column label="操作" width="80">
        <template #default="scope">
          <el-button size="mini" type="danger" @click="permanentlyDelete(scope.row)">彻底删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
// 彻底删除邮件
const permanentlyDelete = (email: any) => {
  ElMessageBox.confirm(
      '确定要彻底删除这封邮件吗？此操作无法撤销。',
      '警告',
      {
...
// 搜索邮件
const searchEmails = () => {
  console.log('搜索垃圾箱:', searchQuery.value)
  // 实际应用中应该调用API进行搜索
  emits('refresh')
}
